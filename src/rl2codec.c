...
/* preparing the OddBlock */
    block_odd_size = 45 + compressed + compressed_mask + palette_size;
	...
	*ptr++ = palette->nEntries;	/* # Palette entries */
...
/* checking the OddBlock for validity */
	...
	return 0;
    if (blob_sz < 45 + compressed + compressed_mask)
	return 0;
    ptr += compressed;
    if (*ptr++ != RL2_DATA_END)
	return 0;
