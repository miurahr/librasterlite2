...
load_dyn_extensions (sqlite3 *db_handle)
{
    ...
    ret = sqlite3_exec (db_handle, "SELECT load_extension('rasterlite2')", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
...
else
{
    if (conn->cache != NULL)
        ...
        rl2_init (db_handle, conn->cache, 0);
}
    ...
    ret = sqlite3_exec (db_handle, "SELECT CreateRasterCoveragesTable()", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
