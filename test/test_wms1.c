...
/* WMS GetCapabilities (valid) */
    url = "http://web.regione.toscana.it/wmsraster/com.rt.wms.RTmap/wms?map=wmsambamm&service=WMS&request=GetCapabilities";
...
/* testing Layer */
    ...
    if (dbl_res != 1.0)
      {
	  fprintf (stderr, "GetWmsLayerMinScaleDenominator: unexpected result %1.6f\n", dbl_res);
	  return -10;
      }
    dbl_res = get_wms_layer_max_scale_denominator (layer);
    if (dbl_res != 5000000.0)
      {
	  fprintf (stderr, "GetWmsLayerMaxScaleDenominator: unexpected result %1.6f\n", dbl_res);
	  return -11;
      }
    ...
    else
    {
        if (maxx != 42.20980)
        {
	    fprintf (stderr, "GetWmsLayerGeoBBox (MaxX): unexpected result %1.8f\n", maxx);
	    return -13;
        }
    }
    ...
    if (get_wms_layer_bbox(layer, "EPSG:3003", &minx, &miny, &maxx, &maxy) != 1)
      {
    ...
    else
    {
        if (maxx != 4678330.0)
        {
	    fprintf (stderr, "GetWmsLayerBBox (MaxX): unexpected result %1.8f\n", maxx);
	    return -17;
        }
    }
    ...
    if (str != NULL)
      {
	  fprintf (stderr, "GetWmsLayerStyleAbstract: unexpected result \"%s\"\n", str);
	  return -21;
      }
...
/* testing Child Layer */
    ...
    if (strcmp(str, "rt_ambamm.idcomuni.rt.poly") != 0)
      {
	  fprintf (stderr, "GetWmsLayerTitle (Child): unexpected result \"%s\"\n", str);
	  return -25;
      }
    ...
    if (int_res == 0)
      {
	  fprintf (stderr, "IsWmsLayerQueryable (Child): unexpected result %d\n", int_res);
	  return -27;
      }
    if (get_wms_layer_geo_bbox(child, &minx, &miny, &maxx, &maxy) != 1)
      {
	  fprintf (stderr, "GetWmsLayerGeoBBox (Child): unexpected result FALSE\n");
	  return -28;
      }
    else
    {
        if (maxx != 42.20980)
        {
	    fprintf (stderr, "GetWmsLayerGeoBBox (Child-MaxX): unexpected result %1.8f\n", maxx);
	    return -29;
        }
    }
    ...
    if (count != 5)
      {
	  fprintf (stderr, "GetWmsLayerCrsCount (Child): unexpected result %d\n", count);
	  return -30;
      }
    ...
    if (count != 8)
      {
	  fprintf (stderr, "GetWmsLayerStyleCount (Child): unexpected result %d\n", count);
	  return -32;
      }
    str =  get_wms_layer_style_name(child, 5);
    if (strcmp(str, "filtro_campo_codcom_not_equal") != 0)
      {
	  fprintf (stderr, "GetWmsLayerStyleName (Child): unexpected result \"%s\"\n", str);
	  return -33;
      }
...
/* testing Service */
    ...
    if (strcmp(str, "Regione Toscana - Direzione Generale Governo del territorio") != 0)
      {
    ...
    if (strcmp(str, "servizio.geografico@regione.toscana.it") != 0)
      {
...
/* WMS GetCapabilities (valid) */
    url = "http://servizigis.regione.emilia-romagna.it/wms/dbtr2008?service=wms&request=getcapabilities&version=130";
    catalog = create_wms_catalog(cache, url, NULL, &err_msg);
    if (catalog == NULL)
      {
	  fprintf (stderr, "Unable to create a WMS Catalog\n");
...
/* testing Service */
    ...
    str = get_wms_url_GetMap_get (catalog);
    if (strcmp(str, "http://servizigis.regione.emilia-romagna.it/wms/dbtr2008") != 0)
      {
	  fprintf (stderr, "GetWmsGetMapGet: unexpected result \"%s\"\n", str);
	  return -138;
      }
    ...
    str = get_wms_contact_person (catalog);
    if (strcmp(str, "Servizio sviluppo dell’amministrazione digitale e sistemi informativi geografici") != 0)
      {
	  fprintf (stderr, "GetContactPerson: unexpected result \"%s\"\n", str);
	  return -144;
      }
    ...
    str = get_wms_contact_position (catalog);
    if (strcmp(str, "Centro Servizi Regionale per l'informazione geografica") != 0)
      {
	  fprintf (stderr, "GetContactPosition: unexpected result \"%s\"\n", str);
	  return -146;
      }
...
/* testing GetFeatureInfo and friends [GML] */
    ...
    ret = sqlite3_open_v2 (":memory:", &sqlite, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL);
    if (ret != SQLITE_OK) {
...
/* testing Feature Member */
    ...
    if (count != 1)
      {
	  fprintf (stderr, "GetFeatureMembersCount (GML): unexpected result %d\n", count);
	  return -202;
      }
    ...
    if (count != 6)
      {
	  fprintf (stderr, "GetFeatureAttributeCount (GML): unexpected result %d\n", count);
	  return -204;
      }
    str = get_wms_feature_attribute_name(ftr, 0);
    if (strcmp(str, "geometryProperty") != 0)
      {
	  fprintf (stderr, "GetFeatureAttributeName (GML-Geom): unexpected result \"%s\"\n", str);
	  return -205;
      }
    ...
    if (count != 501788)
      {
	  fprintf (stderr, "GetFeatureAttributeValue (GML-Geom): unexpected lenght %d\n", count);
	  return -206;
      }
    geom = get_wms_feature_attribute_geometry(ftr, 0);
    if (geom == NULL)
      {
	  fprintf (stderr, "GetFeatureAttributeValue (GML-Geom): unexpected NULL\n");
	  return -207;
      }
    else if (geom->Srid != 3003)
      {
	  fprintf (stderr, "GetFeatureAttributeValue (GML-Geom): unexpected SRID %d\n", geom->Srid);
	  return -208;
      }
    str = get_wms_feature_attribute_name(ftr, 4);
    if (strcmp(str, "CODCOM") != 0)
      {
	  fprintf (stderr, "GetFeatureAttributeName (GML): unexpected result \"%s\"\n", str);
	  return -209;
      }
    str = get_wms_feature_attribute_value(ftr, 4);
    if (strcmp(str, "049014") != 0)
      {
	  fprintf (stderr, "GetFeatureAttributeValue (GML): unexpected result \"%s\"\n", str);
	  return -210;
      }
    geom = get_wms_feature_attribute_geometry(ftr, 4);
    if (geom != NULL)
      {
	  fprintf (stderr, "GetFeatureAttributeValue (GML-Geom): unexpected result\n");
	  return -211;
      }
...
/* testing GetFeatureInfo and friends [XML] */
    ...
    coll = do_wms_GetFeatureInfo_get(url, NULL, "1.3.0", "text/xml", "PRV_Provincia", "CRS:84",
        0, 9.018668,42.615796,13.015085,46.237264, 437, 396, 254, 184, &err_msg);
    if (coll == NULL)
      {
...
/* testing Feature Member */
    ...
    if (count != 1)
      {
	  fprintf (stderr, "GetFeatureMembersCount (XML): unexpected result %d\n", count);
	  return -302;
      }
    ftr = get_wms_feature_member(coll, 0);
    if (ftr == NULL)
    ...
    if (count != 26)
      {
	  fprintf (stderr, "GetFeatureAttributeCount (XML): unexpected result %d\n", count);
	  return -304;
      }
    str = get_wms_feature_attribute_name(ftr, 0);
    if (strcmp(str, "Objectid") != 0)
      {
	  fprintf (stderr, "GetFeatureAttributeName (XML-Geom): unexpected result \"%s\"\n", str);
	  return -305;
      }
    str = get_wms_feature_attribute_value(ftr, 0);
    if (strcmp(str, "329") != 0)
      {
	  fprintf (stderr, "GetFeatureAttributeValue (XML): unexpected result \"%s\"\n", str);
	  return -306;
      }
    geom = get_wms_feature_attribute_geometry(ftr, 0);
    if (geom != NULL)
      {
	  fprintf (stderr, "GetFeatureAttributeValue (XML-Geom): unexpected value\n");
	  return -307;
      }
    str = get_wms_feature_attribute_name(ftr, 2);
    if (strcmp(str, "Nm_prv") != 0)
      {
	  fprintf (stderr, "GetFeatureAttributeName (XML): unexpected result \"%s\"\n", str);
	  return -309;
      }
...
/* GetMap - Pass I [not cached] */
    ...
    rgba = do_wms_GetMap_get(cache, url, NULL, "1.3.0", 
        "rt_ambamm.idcomuni.rt.poly", "EPSG:3003", 0, 1583097.365776,
         ...
         437, 396, "contorno_con_etichette", "image/png", 0, 1, &err_msg);
    if (rgba != NULL)
      {
    ...
    rgba = do_wms_GetMap_get(cache, url, NULL, "1.3.0", 
        "rt_ambamm.idcomuni.rt.poly", "EPSG:3003", 0, 1583097.365776,
         4704017.773106,1611444.731553,4729705.546213,
         437, 396, "contorno_con_etichette", "image/png", 0, 0, &err_msg);
    if (rgba == NULL)
      {
