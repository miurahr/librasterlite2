...
/* WMS GetCapabilities (valid) */
    url = "http://wms.jpl.nasa.gov/wms.cgi?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.1.1";
...
/* testing Service */
    ...
    if (strcmp(str, "WMS Server maintained by JPL, worldwide satellite imagery.") != 0)
      {
...
/* testing TileService layer */
    ...
    else
    {
        if (maxx != 180.0)
        {
	    fprintf (stderr, "GetWmsTiledLayerBBox (MaxX): unexpected result %1.8f\n", maxx);
	    return -41;
        }
    }
    if (get_wms_tiled_layer_tile_size(layer, &width, &height) != 1)
      {
	  fprintf (stderr, "GetWmsTiledLayerTileSize: unexpected result FALSE\n");
	  return -42;
      }
    else
    {
        if (width != 512)
        {
	    fprintf (stderr, "GetWmsTiledLayerTileSize (Width): unexpected result %d\n", width);
	    return -43;
        }
    }
    ...
    if (strcmp(str, "request=GetMap&layers=global_mosaic&srs=EPSG:4326&format=image/jpeg&styles=visual&width=512&height=512&bbox=-180,58,-148,90") != 0)
      {
	  fprintf (stderr, "GetWmsTilePatternSampleURL: unexpected result \"%s\"\n", str);
	  return -50;
      }
    ...
    if (dbl_res != 256.0)
      {
	  fprintf (stderr, "GetTilePatternTileExtentX: unexpected result %1.8f\n", dbl_res);
	  return -55;
      }
    dbl_res = get_wms_tile_pattern_extent_y(layer, 0);
    if (dbl_res != 256.0)
      {
	  fprintf (stderr, "GetTilePatternTileExtentY: unexpected result %1.8f\n", dbl_res);
	  return -56;
      }
    ...
    xurl = get_wms_tile_pattern_request_url(pattern, "http://wms.jpl.nasa.gov/wms.cgi?", -180.0, 90.0);
    if (strcmp(xurl, "http://wms.jpl.nasa.gov/wms.cgi?request=GetMap&layers=global_mosaic&srs=EPSG:4326&format=image/jpeg&styles=visual&width=512&height=512&bbox=-180.000000,90.000000,-148.000000,122.000000") != 0)
      {
	  fprintf (stderr, "GetWmsTilePatternRequestURL: unexpected result %s\n", xurl);
	  return -58;
      }
    sqlite3_free(xurl);
...
/* testing TileService Child Layer */
    ...
    if (strcmp(str, "The full resolution BMNG in jpeg tiled format") != 0)
      {
	  fprintf (stderr, "GetWmsTiledLayerTitle Child: unexpected result \"%s\"\n", str);
	  return -64;
      }
    str = get_wms_tiled_layer_abstract (child);
    if (strcmp(str, "There are twelve styles of this dataset, each one corresponding to a given month of the year, with 960x960 pixel tiles") != 0)
      {
	  fprintf (stderr, "GetWmsTiledLayerAbstract Child: unexpected result \"%s\"\n", str);
	  return -65;
      }
    str = get_wms_tiled_layer_pad (child);
    if (str != NULL)
      {
	  fprintf (stderr, "GetWmsTiledLayerPad Child: unexpected result \"%s\"\n", str);
	  return -66;
      }
    str = get_wms_tiled_layer_bands (child);
    if (str != NULL)
      {
	  fprintf (stderr, "GetWmsTiledLayerBands Child: unexpected result \"%s\"\n", str);
	  return -67;
      }
    str = get_wms_tiled_layer_data_type (child);
    if (str != NULL)
      {
	  fprintf (stderr, "GetWmsTiledLayerDataType Child: unexpected result %s\n", str);
	  return -68;
      }
    if (get_wms_tiled_layer_bbox(child, &minx, &miny, &maxx, &maxy) != 1)
      {
	  fprintf (stderr, "GetWmsTiledLayerBBox Child: unexpected result FALSE\n");
	  return -69;
      }
    else
    {
        if (maxx != DBL_MAX)
        {
	    fprintf (stderr, "GetWmsTiledLayerBBox (Child MaxX): unexpected result %1.8f\n", maxx);
	    return -70;
        }
    }
    if (get_wms_tiled_layer_tile_size(child, &width, &height) != 0)
      {
	  fprintf (stderr, "GetWmsTiledLayerTileSize Child: unexpected result TRUE\n");
	  return -71;
      }
...
/* GetMap - Pass I [not cached] */
    url = "http://wms.jpl.nasa.gov/wms.cgi?request=GetMap&layers=global_mosaic&srs=EPSG:4326&format=image/jpeg&styles=visual&width=512&height=512&bbox=10.312500,42.812500,10.375000,42.875000";
    rgba = do_wms_GetMap_TileService_get(cache, url, NULL, 512, 512, 1, &err_msg);
    if (rgba != NULL)
      {
	  fprintf (stderr, "GetMap TileService (not cached): unexpected result\n");
	  return -87;
      }
    rgba = do_wms_GetMap_TileService_get(cache, url, NULL, 512, 512, 0, &err_msg);
    if (rgba == NULL)
      {
	  fprintf (stderr, "GetMap TileService (not cached): unexpected NULL\n");
	  return -88;
      }
     free(rgba);
